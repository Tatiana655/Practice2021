...
def my_parafac(tensor,rank,eps,B,C):
    ...
    Предназначен для искуственных примеров, на реальных данных лучше из коробки взять
...
def NanLine(tuple_matrix, line_width, coef_line, mode = 'zero'):
    '''
    remove line _y = coef_line * _x lines from data using mode::
    mode::  'zero' - replaces vals with ZEROS
            'Inf' - replaces vals with np.INF
            'nzero' - replaces with "mean-line"
...
def grease_line(line,b1,b2):
    '''
    replaces the values in line with the average using the b1,b2 boundaries
    :param line: 1-way array
    :param b1: left border
    :param b2: right border
    :return: NEW line
    '''
    z1 = 0.
    z2 = 0.
...
def fill_nan(tuple_matrix):
    '''
    Fills infinities with middle lines using intermediate points
    :param tuple_matrix: 2-way array
    :return: NEW 2-way array
    '''
    x = tuple_matrix[0]
    y = tuple_matrix[1]
    z = tuple_matrix[2]
...
def fill_all_nan(tuple_matrix):
    '''
    Fills infinities with middle lines grease (smearing) intermediate points
    :param tuple_matrix: 2-way array
    :return: NEW 2-way array
    '''
    x = tuple_matrix[0]
...
def read_tensor(folder_path):
    '''
    Create TENSOR from the path
    NB!: YOU MUST check the same steps.
    :param folder_path: example: "C:\\Users\\Tatiana\\Desktop\\Baikal\\"
    :return: Tuple_tensor
    '''
...
def erase_Reyleigh(data, width1, width2, mode="nzero"):
    ...
    :param width1: width y=x line in OX section
    :param width2: width y=2x line in OX section
    :param mode: "zero", "nzero", "Inf"
    :return:
    '''
    x = data[0]
    y = data[1]
...
def show_data(data,h,w,title = "DATA",num1=-1, num2=-1,names = None):
    '''
    Show tensor slice (slib)
    :param data:  tensor (+em +ex)
    :param h: number of rows of pictures
    :param w: number of columns of pictures
        NB!: h*w >= count_of_slice(tensor)
    :param title: window_title
    :param num1: which number to start print; -1 - first slice
    :param num2: which number to stop print; -1 - last slice (include)
    :param names: picture (slice names)
    :return:
    '''
    x = data[0]
    y = data[1]
...
def show_loadings(data,rank,tol= 1e-6,iter_max = 1000):
    '''
    Show parafac factor-components - Emission, Exсitation, Concentration
    :param data: data tensor
    :param rank: rank (count of component)
    :param tol:
    :param iter_max:
    :return:
    '''
    ...
    ax.grid()
    ax.plot(np.linspace(1, len(tensor_data) , len(tensor_data) , endpoint=True), model[1][0])
